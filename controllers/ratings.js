...
// makeModels param lets you inject your models manually
...
const addRatings = async (req, res, next) => {
    try {
        ...
        const existingRating = await ratingsModel.findOne({ student: studentId, week });
        if (existingRating) {
            return res.status(400).json({ message: "Student has already been rated for this week" });
        }
...
const deleteRatingss = async (req, res, next) => {
    try {
        ...
        const ratingToDelete = await ratingsModel.findOne({ student: studentId, week: Number(week) });
        if (!ratingToDelete) return next(ApiError.notFound("Rating for specified week not found"));

        await ratingsModel.findByIdAndDelete(ratingToDelete._id);

        const remainingRatings = await ratingsModel.find({ student: studentId });

        const validTotals = remainingRatings.map(r => r.total).filter(val => typeof val === 'number' && !isNaN(val));
...
const updateRatings = async (req, res, next) => {
    try {
        ...
        const rating = await ratingsModel.findOne({ student: studentId, week: Number(week) });
        if (!rating) {
            return next(ApiError.notFound('Rating not found for the specified week'));
        }
